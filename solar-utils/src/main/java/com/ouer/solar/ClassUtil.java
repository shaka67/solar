...
public abstract class ClassUtil {
    ...
    public static Class<?> getPrimitiveType(Class<?> type) {
        return getPrimitiveType(type, false);
    }

    public static Class<?> getPrimitiveType(Class<?> type, boolean ignore) {
        if (type == null) {
            return null;
        }

        Class<?> clazz = getPrimitiveType(type.getName());
        if (clazz != null) {
            return clazz;
        }

        return (ignore) ? type : null;
    }
    ...
    /**
     * 取得primitive类型的最大值。如果不是primitive，则返回<code>null</code>。
     * <p>
     * 如果<code>clazz</code>为<code>null</code>，则返还 <code>null</code>
     * <p>
     * 例如：
     ...
     * 
     * <pre>
     * ClassUtil.getPrimitiveDefaultValue(int.class) = Integer.MAX_VALUE;
     * ClassUtil.getPrimitiveDefaultValue(boolean.class) = true;
     * ClassUtil.getPrimitiveDefaultValue(char.class) = Character.MAX_VALUE;
     * ;
     * </pre>
     * <p/>
     * </p>
     */
    public static <T> T getPrimitiveMaxValue(Class<T> type) {
        if (type == null) {
            return null;
        }
    ...
    /**
     * 取得primitive类型的最小值。如果不是primitive，则返回<code>null</code>。
     * <p>
     * 如果<code>clazz</code>为<code>null</code>，则返还 <code>null</code>
     * <p>
     * 例如：
     ...
     * 
     * <pre>
     * ClassUtil.getPrimitiveDefaultValue(int.class) = Integer.MIN_VALUE;
     * ClassUtil.getPrimitiveDefaultValue(boolean.class) = false;
     * ClassUtil.getPrimitiveDefaultValue(char.class) = Character.MIN_VALUE;
     * ;
     * </pre>
     * <p/>
     * </p>
     */
    public static <T> T getPrimitiveMinValue(Class<T> type) {
        if (type == null) {
            return null;
        }
    ...
    /**
     * 判断<code>thisClass</code>是否为<code>targetInterface</code>的实现类
     * 
     * @param thisClass 实现类
     * @param targetInterface 接口类
     * @return 是否为其实现类
     */
    ...
    public static boolean isAssignable(Class<?> clazz, Class<?> fromClass) {
        ...
        if (clazz.isPrimitive()) {
            return ASSIGNMENT_TABLE.get(clazz).contains(fromClass);
        }

        return false;
    }

    private static final Map<Class<?>, Set<Class<?>>> ASSIGNMENT_TABLE = CollectionUtil.createHashMap();

    static {
        // boolean可以接受：boolean
        ASSIGNMENT_TABLE.put(boolean.class, assignableSet(boolean.class));

        // byte可以接受：byte
        ASSIGNMENT_TABLE.put(byte.class, assignableSet(byte.class));

        // char可以接受：char
        ASSIGNMENT_TABLE.put(char.class, assignableSet(char.class));

        // short可以接受：short, byte
        ASSIGNMENT_TABLE.put(short.class, assignableSet(short.class, byte.class));

        // int可以接受：int、byte、short、char
        ASSIGNMENT_TABLE.put(int.class, assignableSet(int.class, byte.class, short.class, char.class));

        // long可以接受：long、int、byte、short、char
        ASSIGNMENT_TABLE.put(long.class, assignableSet(long.class, int.class, byte.class, short.class, char.class));

        // float可以接受：float, long, int, byte, short, char
        ASSIGNMENT_TABLE.put(float.class,
     ...
     * 
     * <pre>
     *  ClassUtil.getSimpleClassName(Boolean.TRUE) = "Boolean"
     *  ClassUtil.getSimpleClassName(new Boolean[10]) = "Boolean[]"
     *  ClassUtil.getSimpleClassName(new int[1][2]) = "int[][]"
     * </pre>
     * <p>
     ...
     * @return 简单类名，如果对象为 <code>null</code> ，则返回 <code>null</code>
     */
    public static String getSimpleClassName(Object object, boolean processInnerClass) {
        if (object == null) {
            return null;
        }
    ...
    public static Class<?>[] objects2Classes(final Object...objects) {
        if (ArrayUtil.isEmpty(objects)) {
            return Emptys.EMPTY_CLASS_ARRAY;
        }
        ...
        for (int i = 0; i < objectLength; i++) {
            ...
            classes[i] = ClassUtil.getPrimitiveType(objects[i].getClass(), true);
        }

        return classes;
    }

    public static Class<?>[] toPrimitive(Class<?>...classes) {
        if (ArrayUtil.isEmpty(classes)) {
            return Emptys.EMPTY_CLASS_ARRAY;
        }

        Class<?>[] primitiveClasses = new Class[classes.length];
        for (int i = 0; i < classes.length; i++) {
            primitiveClasses[i] = ClassUtil.getPrimitiveType(classes[i], true);
        }

        return primitiveClasses;
    }

    public static Class<?>[] toWrapper(Class<?>...classes) {
        if (ArrayUtil.isEmpty(classes)) {
            return Emptys.EMPTY_CLASS_ARRAY;
        }
        ...
        Class<?>[] primitiveClasses = new Class[classes.length];
        for (int i = 0; i < classes.length; i++) {
            primitiveClasses[i] = ClassUtil.getWrapperTypeIfPrimitive(classes[i]);
        }
