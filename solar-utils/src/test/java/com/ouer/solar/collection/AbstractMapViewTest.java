...
public abstract class AbstractMapViewTest extends AbstractTest {
    ...
    /**
     * 测试Object.equals()方法. 但并非所有的collection对象都定义了此方法. 如果当前测试的collection对象不支持比较操作, 则
     * createCollectionToCompareWith()应该返回null.
     */
    @Test
    @SuppressWarnings("unchecked")
    public void equals_() {
    ...
    /**
     * 测试Object.hashCode()方法. 该方法总是和Object.equals()相关的(相等的对象有 相同的hashCode), 因此,类似equals_(), hashCode_通过判断
    ...
    /**
     * 从<code>Map</code>中取得view. 可能是<code>Map.entrySet()</code>, <code>Map.keySet()</code> 和<code>Map.values()</code>
     * 等方法返回的结果.
     * 
    ...
    /**
     * 创建一个可以和当前测试的view相比较的对象. 对于<code>Map.values()</code>返回的 <code>Collection</code>对象, 没有定义<code>equals</code> 和
